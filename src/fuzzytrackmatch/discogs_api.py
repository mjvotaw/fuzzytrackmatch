"""A small Discogs API client that maps responses to our dataclasses.

Usage:
    client = DiscogsApiClient(token="...", user_agent="my-app/0.1")
    release = client.get_release(12345)

This client is intentionally small: it uses requests.Session, ensures the
required headers (User-Agent and Authorization), parses rate-limit headers
from responses and performs a simple retry/sleep when the rate-limit is
exhausted or on 429 responses.
"""
